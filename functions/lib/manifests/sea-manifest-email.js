...
exports.slSendSeaManifestInvoiceEmail = void 0;
const https_1 = require("firebase-functions/v2/https");
const v2_1 = require("firebase-functions/v2");
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const email_service_1 = require("../email/email-service");
function loadSeaInvoiceTemplate() {
    try {
        const templatePath = path.join(__dirname, '..', '..', 'templates', 'sea-invoice-email.html');
        return fs.readFileSync(templatePath, 'utf8');
    }
    catch (error) {
        v2_1.logger.error('[loadSeaInvoiceTemplate] Error loading template:', error);
...
exports.slSendSeaManifestInvoiceEmail = (0, https_1.onCall)({
    cors: true,
    maxInstances: 10,
}, async (request) => {
    ...
    v2_1.logger.info('[slSendSeaManifestInvoiceEmail] Sending sea invoice email', {
    ...
    try {
        ...
        if (!result.success) {
            v2_1.logger.error('[slSendSeaManifestInvoiceEmail] Failed to send email', { error: result.error });
        ...
        v2_1.logger.error('[slSendSeaManifestInvoiceEmail] Error', { error: error.message });
        throw new https_1.HttpsError('internal', error.message || 'Unknown error');
    }
});
//# sourceMappingURL=sea-manifest-email.js.map