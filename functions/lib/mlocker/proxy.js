...
function rawBinaryRequest(options, body) {
    return new Promise((resolve, reject) => {
        const req = https.request(options, (res) => {
            const chunks = [];
...
async function ensurePortalSession() {
    ...
    if (!loginOk) {
        throw new Error(`Portal login failed: status=${loginRes.status} location=${redirectLoc || "none"}`);
    }
 ...
 */
function generateTrackingVariants(input) {
    ...
    if (/^\d{30,40}$/.test(cleaned)) {
        ...
        for (let start = 0; start <= 22; start++) {
            ...
            if (seg.length >= 12 && seg.length <= 22)
                add(seg);
        }
    }
...
async function fetchOneTracking(candidate, authHeaders, isRetry = false) {
 ...
 */
function detectMissingDestination(destino, codigoCliente, manifestId) {
    if (destino && destino.toUpperCase() === "SD")
        return true;
    if (codigoCliente && codigoCliente.toUpperCase() === "SD")
        return true;
    if (manifestId) {
        ...
        if (n.includes("SINDESTINO"))
            return true;
        const suffix = n.match(/[A-Z]+$/)?.[0] ?? "";
        if (suffix === "DANS" || suffix === "SD" || suffix === "SIND")
            return true;
    }
    return false;
}
...
async function trackPackage(trackingNumber) {
    ...
    const parallelResults = await Promise.all(topVariants.map(async (candidate) => ({ candidate, ...(await fetchOneTracking(candidate, authHeaders)) })));
    ...
    const manifestId = info?.idManifest ?? "";
    const destino = info?.destino ?? "";
    const clientCode = info?.codigoCliente ?? "";
    return {
        found: true,
        trackingNumber: info?.trackingNumber ?? matchedTracking,
        originalInput: normalized,
        // Core package fields Nova needs
        destination: (destino || info?.destinoCompleto) ?? "",
        destinationFull: info?.destinoCompleto ?? "",
        shipper: info?.shipper ?? "",
        shipperDescription: info?.shipperDescripcion ?? "",
        weight: info?.peso ?? 0,
        pieces: info?.bultos ?? 0,
        customerCode: clientCode,
        customerName: info?.nombreCliente ?? "",
        manifestId,
        description: info?.descripcionArticulo ?? "",
        invoice: info?.factura ?? "",
        notes: info?.notas ?? "",
...
async function listManifests(params) {
    ...
    const recordsTotal = parsed.recordsTotal || 0;
    ...
    return {
        ...
        manifests: data.map((m) => {
            ...
            const rawType = String(m.TipoManifiesto ?? m.tipoManifiesto ?? m.Tipo ?? m.tipo ?? "");
            const suffix = id.match(/[A-Z]+$/)?.[0] ?? "";
            const manifestType = rawType || (suffix === "DANP" ? "Permisos" :
                suffix === "PER" ? "Permisos" :
...
async function getManifestDetail(manifestId) {
    const cookies = await ensurePortalSession();
    ...
    if (tbodyMatch) {
        ...
        for (const rowMatch of rowMatches) {
            ...
            const cellTexts = cells.map((c) => c[1].replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim());
...
async function downloadManifestExcel(manifestId, type = "summary") {
    const cookies = await ensurePortalSession();
    const formData = querystring.stringify({
        type,
    ...
    try {
        ...
        console.error(`[MLockerProxy] Storage upload failed: ${storageErr?.message ?? storageErr}`);
    }
    return {
        success: true,
        manifestId,
...
// ── Callable function ─────────────────────────────────────────────────────────
exports.slMLockerProxy = (0, https_1.onCall)({
    cors: true,
    timeoutSeconds: 60,
    memory: "256MiB",
}, async (request) => {
    ...
    if (!data || !data.action) {
        throw new https_1.HttpsError("invalid-argument", "Missing required field: action");
    }
    try {
        switch (data.action) {
            case "track_package": {
            ...
            case "get_manifest_detail": {
                if (!data.manifestId) {
                    throw new https_1.HttpsError("invalid-argument", "manifestId is required");
                }
            ...
            default:
                throw new https_1.HttpsError("invalid-argument", `Unknown action: ${data.action}`);
        }
    }
    catch (err) {
        ...
        throw new https_1.HttpsError("internal", `MLocker proxy error: ${msg}`);
    }
});
