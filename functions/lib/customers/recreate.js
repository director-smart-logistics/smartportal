...
exports.slRecreateSp2UserAccount = exports.slRecreateCustomerBySlCode = void 0;
/**
...
exports.slRecreateCustomerBySlCode = (0, https_1.onCall)({ cors: true, memory: "256MiB", timeoutSeconds: 30 }, async (request) => {
    if (!request.auth) {
    ...
    if (!slCode)
        throw new https_1.HttpsError("invalid-argument", "slCode requerido");
    if (!email)
        throw new https_1.HttpsError("invalid-argument", "email requerido");
    if (!firstName)
        throw new https_1.HttpsError("invalid-argument", "firstName requerido");
    // ── Refuse if already exists ──────────────────────────────────────────
    const docRef = firebase_1.db.collection("customers").doc(slCode);
    const existing = await docRef.get();
    if (existing.exists && !data.force) {
        throw new https_1.HttpsError("already-exists", `El cliente ${slCode} ya existe en SP1. No hay nada que recuperar.`);
    }
    ...
    try {
        await firebase_1.db.collection("_admin_audit").add({
        ...
        v2_1.logger.warn("[slRecreateCustomerBySlCode] audit log failed", {
            slCode, error: err.message,
        });
    }
    v2_1.logger.info("[slRecreateCustomerBySlCode] customer recreated", {
...
function getSp2Admin() {
    ...
    const app = existing || (0, app_1.initializeApp)({ projectId: SP2_PROJECT_ID }, appName);
    return {
        sp2Db: (0, firestore_1.getFirestore)(app),
        sp2Auth: (0, auth_1.getAuth)(app),
    };
}
function normalizeStr(text) {
    return text.normalize("NFD").replace(/[\u0300-\u036f]/g, "");
}
function generateSearchTokens(firstName, lastName) {
...
exports.slRecreateSp2UserAccount = (0, https_1.onCall)({ cors: true, memory: "256MiB", timeoutSeconds: 60 }, async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "Must be authenticated");
    }
    const role = request.auth.token.role || "";
    ...
    const customerSnap = await firebase_1.db.collection("customers").doc(slCodeClean).get();
    if (!customerSnap.exists) {
        throw new https_1.HttpsError("not-found", `Customer ${slCodeClean} not found in SP1`);
    }
    ...
    if (!email) {
        throw new https_1.HttpsError("invalid-argument", "Customer record must have a valid email to recreate account in SP2");
    }
    if (!firstName) {
        throw new https_1.HttpsError("invalid-argument", "Customer record must have a valid firstName to recreate account in SP2");
    }
    ...
    try {
        ...
        try {
            ...
            v2_1.logger.info(`[slRecreateSp2UserAccount] Auth user already exists in SP2 with email: ${email}`, { uid: authUser.uid });
        }
        catch (authErr) {
            if (authErr.code === "auth/user-not-found") {
                ...
                v2_1.logger.info(`[slRecreateSp2UserAccount] Auth user created in SP2`, { uid: authUser.uid });
            }
            else {
                throw authErr;
            }
        ...
        await sp2Db.runTransaction(async (tx) => {
            ...
            if (userDocSnap.exists) {
                ...
                if (!existingSlCode || existingSlCode === slCodeClean) {
                    ...
                    v2_1.logger.info(`[slRecreateSp2UserAccount] User document exists in SP2 Firestore for UID: ${uid}. Assigning/updating slCode to ${slCodeClean}`);
                }
                else {
        ...
        try {
            ...
            v2_1.logger.info(`[slRecreateSp2UserAccount] Standard password reset link generated for email: ${email}`);
        }
        catch (linkErr) {
            v2_1.logger.warn(`[slRecreateSp2UserAccount] Failed to generate reset link (non-fatal)`, linkErr);
        }
        // 6. Audit logs in SP1
        try {
            await firebase_1.db.collection("_admin_audit").add({
            ...
            v2_1.logger.warn("[slRecreateSp2UserAccount] audit log failed", err);
        }
        return {
            success: true,
        ...
        v2_1.logger.error(`[slRecreateSp2UserAccount] Failed to recreate account in SP2 for ${slCodeClean}`, err);
        return {
            success: false,
