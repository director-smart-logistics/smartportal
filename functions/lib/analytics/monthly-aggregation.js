...
const TIER_LABELS = { basic: 'Basic', smart: 'Smart', premium: 'Premium', business: 'Business' };
const STATUS_LABELS_DEMO = { active: 'Activo', inactive: 'Inactivo', suspended: 'Suspendido', deleted: 'Eliminado' };
...
async function fetchHybridCollection(collectionName, start, end, selectFields) {
    let queryTs = firebase_1.db.collection(collectionName)
        .where("createdAt", ">=", start)
        .where("createdAt", "<=", end);
    let queryStr = firebase_1.db.collection(collectionName)
        .where("createdAt", ">=", start.toISOString())
        .where("createdAt", "<=", end.toISOString());
    if (selectFields && selectFields.length > 0) {
        queryTs = queryTs.select(...selectFields);
        queryStr = queryStr.select(...selectFields);
    }
...
async function aggregateMonthlyData(month, includeTrend = true) {
    ...
    const [packagesRaw, invoices, newCustomers, preAlertsRaw, allCustomersSnap, allPackagesWithCustSnap] = await Promise.all([
        ...
        firebase_1.db.collection("customers").select("encomiendaServiceName", "slCode", "ruta", "status", "isVerified", "sp2CreatedAt", "memberSince", "createdAt", "migratedFromWordPress", "migratedFromLegacy", "wpUserId").get(),
        firebase_1.db.collection("packages").select("customerId", "slCode").get(),
    ]);
    ...
    let regularWeight = 0;
    let permitWeight = 0;
    const packagePermitMap = new Map();
    packages.forEach(p => {
        ...
        // Populate permit map
        if (p.id)
            packagePermitMap.set(p.id, isPermit);
    ...
    const customersWithPackages = new Set();
    ...
    allCustomersSnap.docs.forEach(doc => {
        ...
        if (slCode) {
            if (service)
                customerServiceMap.set(slCode, service);
            if (route)
                customerRouteMap.set(slCode, route);
        }
        if (doc.id) {
            if (service)
                customerServiceMap.set(doc.id, service);
            if (route)
                customerRouteMap.set(doc.id, route);
        }
    });
    ...
    packages.forEach(p => {
        ...
        if (id)
            activeCustomersSet.add(String(id).toUpperCase().trim());
    });
    invoices.forEach(i => {
    ...
    const newCustomersFiltered = newCustomers.filter((c) => {
    ...
    function resolveCustomerEarliestRegDate(d) {
        if (!d)
            return new Date(0);
    ...
    if (includeTrend && missingIdsArray.length > 0) {
        ...
        for (let i = 0; i < missingIdsArray.length; i += chunkSize) {
            ...
            const refs = chunk.map(id => firebase_1.db.collection("packages").doc(id));
            try {
                const snaps = await firebase_1.db.getAll(...refs);
    ...
    const invoicesByRouteMap = new Map();
    invoices.forEach(inv => {
        let route = 'Sin ruta';
        if (inv.items && Array.isArray(inv.items)) {
    ...
    packages.forEach(p => {
        ...
        if (mainTrk)
            trksToCheck.push(mainTrk);
        if (Array.isArray(p.groupedTrackings)) {
            p.groupedTrackings.forEach((t) => {
                if (t)
                    trksToCheck.push(String(t).toUpperCase().trim());
            });
        }
        ['originalTracking', 'externalTracking', 'supplierTracking', 'carrierTracking'].forEach(f => {
            if (p[f])
                trksToCheck.push(String(p[f]).toUpperCase().trim());
        });
        let detected = 'Otros';
    ...
    const encomiendaCounts = new Map();
    packages.forEach(p => {
    ...
    if (includeTrend) {
        ...
        const trendResults = await Promise.all(months.map(async (mStr) => {
            ...
            try {
                const docSnap = await firebase_1.db.collection("monthly_analytics").doc(mStr).get();
                if (docSnap.exists) {
            ...
            try {
                ...
                await firebase_1.db.collection("monthly_analytics").doc(mStr).set(computed).catch(() => { });
                return {
                    period: mStr,
...
//# sourceMappingURL=monthly-aggregation.js.map