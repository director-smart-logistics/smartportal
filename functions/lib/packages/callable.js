...
exports.slDeletePackage = exports.slUpdatePackageStatus = exports.slUpdatePackage = exports.slCreatePackage = exports.slGetPackageByTracking = exports.slGetPackage = exports.slListPackages = void 0;
...
function pushStatusToSP2(pkg) {
    ...
    if (!secret || !pkg.trackingNumber || !pkg.slCode)
        return;
    fetch(url, {
        method: 'POST',
        headers: {
        ...
        v2_1.logger.warn('[pushStatusToSP2] Non-blocking sync error', {
...
function generateTrackingSuffixes(trackingNumber) {
...
function buildPackageSearchIndex(customerName, slCode, trackingNumber) {
    const tokens = new Set([
...
exports.slListPackages = (0, https_1.onCall)({ cors: true }, async (request) => {
    if (!request.auth) {
    ...
    if (q) {
        ...
        const idSnaps = await Promise.all([...new Set([qUpper, q, qLower])].map((v) => firebase_1.db.collection("packages").doc(v).get()));
        idSnaps.forEach(addDocSnap);
        // ── 2. Smart index-based fan-out (parallel, no collection scan) ──────────
        const indexPromises = [];
        if (isSlCode) {
            // Exact slCode match
            indexPromises.push(firebase_1.db.collection("packages")
                .where("slCode", "==", qUpper)
        ...
        let listQuery = firebase_1.db.collection("packages")
            .orderBy("createdAt", "desc")
            .limit(500);
        if (status) {
            listQuery = firebase_1.db.collection("packages")
                .where("status", "==", status)
                .orderBy("createdAt", "desc")
                .limit(500);
        }
...
exports.slGetPackage = (0, https_1.onCall)({ cors: true }, async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "User must be authenticated");
    }
    const { packageId } = request.data;
    if (!packageId) {
        throw new https_1.HttpsError("invalid-argument", "Package ID is required");
    }
    const packageDoc = await firebase_1.db.collection("packages").doc(packageId).get();
    if (!packageDoc.exists) {
        throw new https_1.HttpsError("not-found", "Package not found");
    }
    const data = packageDoc.data();
    const historySnapshot = await firebase_1.db.collection("packages")
        .doc(packageId)
...
exports.slGetPackageByTracking = (0, https_1.onCall)({ cors: true }, async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "User must be authenticated");
    }
    const { tracking } = request.data;
    if (!tracking) {
        throw new https_1.HttpsError("invalid-argument", "Tracking number is required");
    }
...
exports.slCreatePackage = (0, https_1.onCall)({ cors: true }, async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "User must be authenticated");
    }
    const data = request.data;
    if (!data.trackingNumber || !data.customerName) {
        throw new https_1.HttpsError("invalid-argument", "trackingNumber and customerName are required");
    }
    const existingTracking = await firebase_1.db.collection("packages")
        .where("trackingNumber", "==", data.trackingNumber)
        .limit(1)
        .get();
    if (!existingTracking.empty) {
        throw new https_1.HttpsError("already-exists", "A package with this tracking number already exists");
    }
    const now = firebase_1.admin.firestore.FieldValue.serverTimestamp();
    const packageRef = firebase_1.db.collection("packages").doc();
    const packageData = {
        id: packageRef.id,
    ...
    const searchIndex = buildPackageSearchIndex(data.customerName, data.slCode, data.trackingNumber);
...
exports.slUpdatePackage = (0, https_1.onCall)({ cors: true }, async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "User must be authenticated");
    }
    ...
    const needsReindex = updateFields.customerName != null ||
        updateFields.slCode != null ||
        updateFields.trackingNumber != null;
    if (needsReindex) {
        const newName = updateFields.customerName ?? currentData?.customerName ?? "";
        const newSlCode = updateFields.slCode ?? currentData?.slCode;
        const newTracking = updateFields.trackingNumber ?? currentData?.trackingNumber ?? "";
    ...
    await firebase_1.db.collection("packages").doc(packageId).update(updateData);
    ...
    try {
        ...
        await firebase_1.db.collection("audit_logs").add({
            ...
            source: "server",
            metadata: {
                trackingNumber: updateFields.trackingNumber ?? currentData?.trackingNumber ?? currentData?.tracking ?? "",
                slCodeChanged,
        ...
        v2_1.logger.warn("[slUpdatePackage] Non-blocking audit logging failed", { error: auditErr.message });
    }
    if (statusChanged) {
        await firebase_1.db.collection("packages").doc(packageId).collection("trackingHistory").add({
    ...
    if (slCodeChanged || statusChanged) {
        ...
        pushStatusToSP2({
            trackingNumber: updateFields.trackingNumber ?? currentData?.trackingNumber ?? currentData?.tracking ?? packageId,
            slCode: updateFields.slCode ?? currentData?.slCode,
            status: updateFields.status ?? currentData?.status ?? 'received',
            weight: updateFields.weight ?? currentData?.weight,
            description: updateFields.description ?? currentData?.description,
            ruta: updateFields.ruta ?? currentData?.ruta,
            manifestNumber: updateFields.manifestNumber ?? currentData?.manifestNumber,
            requiresPermit: updateFields.requiresPermit ?? currentData?.requiresPermit,
            calculatedCost: updateFields.calculatedCost ?? currentData?.calculatedCost,
            cost: updateFields.cost ?? currentData?.cost,
            currency: updateFields.currency ?? currentData?.currency,
...
exports.slUpdatePackageStatus = (0, https_1.onCall)({ cors: true }, async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "User must be authenticated");
    }
    const { packageId, status, location, notes, deliverySignature, paymentCollected } = request.data;
    if (!packageId || !status) {
        throw new https_1.HttpsError("invalid-argument", "packageId and status are required");
    }
    const packageDoc = await firebase_1.db.collection("packages").doc(packageId).get();
    if (!packageDoc.exists) {
        throw new https_1.HttpsError("not-found", "Package not found");
...
exports.slDeletePackage = (0, https_1.onCall)({ cors: true }, async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "User must be authenticated");
    }
    ...
    await firebase_1.db.collection("packages").doc(packageId).update({
        status: "cancelled",
