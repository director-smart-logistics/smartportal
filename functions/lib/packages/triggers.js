...
exports.onPackageWritten = void 0;
/**
...
async function computeDesiredLink(tracking, clientSlCode) {
    if (!tracking || !clientSlCode)
        return { id: null, number: null, status: null };
...
exports.onPackageWritten = (0, firestore_1.onDocumentWritten)({
    ...
    if (!after) {
        ...
        try {
            await firebase_1.db.collection("manifest_encomiendas").doc(trackingUpper).delete();
            v2_1.logger.info("[package-trigger] deleted from manifest_encomiendas on package deletion", { pkgId: trackingUpper });
        }
        catch (err) {
            v2_1.logger.warn("[package-trigger] failed to delete from manifest_encomiendas on package deletion", {
                pkgId: trackingUpper,
                error: err.message,
            });
        }
    ...
    if (isEncomienda) {
        ...
        if (hasChanged) {
            ...
            try {
                await firebase_1.db.collection("manifest_encomiendas").doc(trackingUpper).set(encomiendaPayload, { merge: true });
                v2_1.logger.info("[package-trigger] manifest_encomiendas synced", { pkgId: trackingUpper, status: after.status });
            }
            catch (err) {
                v2_1.logger.warn("[package-trigger] manifest_encomiendas sync failed", {
                    pkgId: trackingUpper,
                    error: err.message,
                });
            }
        ...
        try {
            await firebase_1.db.collection("manifest_encomiendas").doc(trackingUpper).delete();
            v2_1.logger.info("[package-trigger] deleted from manifest_encomiendas on route change", { pkgId: trackingUpper });
        }
        catch (err) {
            v2_1.logger.warn("[package-trigger] manifest_encomiendas delete on route change failed", {
                pkgId: trackingUpper,
                error: err.message,
            });
        }
    ...
    if (before && onlyManagedDiff(before, after))
        return;
    ...
    if (linkUnchanged && statusUnchanged)
        return;
    try {
        const updateData = {
        ...
        if (!statusUnchanged) {
            ...
            updateData.statusHistory = firestore_2.FieldValue.arrayUnion({
        ...
        v2_1.logger.info("[package-trigger] link and status enforced", {
            pkgId, tracking, sl,
        ...
        v2_1.logger.warn("[package-trigger] link/status update failed", {
            pkgId, tracking, error: err.message,
        });
    }
});
