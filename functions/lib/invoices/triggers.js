...
exports.onInvoiceWritten = void 0;
...
function isTransitoria(pkg) {
    ...
    if (mId)
        return mId === "consolidacion_transitoria";
    if (mNum)
        return mNum === "consolidacion_transitoria";
    if (mnf)
...
function hasRelevantDiff(before, after) {
    if (!before && after)
        return true;
    if (before && !after)
        return true;
...
function normalizeInvoiceLegacyFields(data) {
    const items = Array.isArray(data.invoiceItems) && data.invoiceItems.length > 0
        ? data.invoiceItems
    ...
    const trackingsFromItems = Array.from(new Set(items
    ...
    // every benign update.
    const patch = {};
    if (Number(data.amount ?? -1) !== canonicalTotal)
        patch.amount = canonicalTotal;
    if (Number(data.subtotal ?? -1) !== canonicalSubtotal)
        patch.subtotal = canonicalSubtotal;
    if (Number(data.iva ?? -1) !== canonicalIva)
        patch.iva = canonicalIva;
...
function buildSp2Payload(data, invoiceId, overrideStatus) {
...
async function pushToSp2(payload, contextLabel) {
    const secret = process.env.SP2_SYNC_SECRET || "";
    if (!secret) {
        v2_1.logger.warn(`[invoice-trigger:${contextLabel}] SP2_SYNC_SECRET missing — skipping push`);
        return;
    }
    try {
        ...
        if (!res.ok) {
            ...
            v2_1.logger.warn(`[invoice-trigger:${contextLabel}] SP2 sync HTTP ${res.status}`, { body: body.slice(0, 300) });
            return;
        }
        const body = await res.json().catch(() => null);
        const outcome = body?.results?.[0]?.outcome ?? "unknown";
        v2_1.logger.info(`[invoice-trigger:${contextLabel}] SP2 sync ok`, {
            invoiceId: payload.id, outcome,
        });
    }
    catch (err) {
        v2_1.logger.warn(`[invoice-trigger:${contextLabel}] SP2 sync error`, { error: err.message });
    }
}
...
async function findCandidateInvoicesForTracking(tracking, clientSlCode) {
    ...
    for (const snap of snaps) {
        ...
        for (const d of snap.docs) {
            ...
            if (clientSlCode && invSl && invSl !== clientSlCode)
                continue;
...
async function enforcePackageLinksForInvoice(invoiceId, invoiceData, beforeData) {
    const after = invoiceData;
    const status = String(after.status || "draft").toLowerCase();
    const afterTracks = Array.isArray(after.trackingNumbers) && after.trackingNumbers.length > 0
        ? after.trackingNumbers
        : (Array.isArray(after.invoiceItems)
            ? after.invoiceItems.map((it) => (it?.trackingNumber || it?.tracking || "")).filter((t) => !!t)
            : []);
    const beforeTracks = beforeData
        ? (Array.isArray(beforeData.trackingNumbers) && beforeData.trackingNumbers.length > 0
            ...
            : (Array.isArray(beforeData.invoiceItems)
                ? beforeData.invoiceItems.map((it) => (it?.trackingNumber || it?.tracking || "")).filter((t) => !!t)
                : []))
        : [];
    ...
    const snaps = await Promise.all(chunks.map(chunk => firebase_1.db.collection("packages").where("trackingNumber", "in", chunk).get()));
    for (const snap of snaps) {
    ...
    const invoiceCandidatesCache = new Map();
    const getInvoiceCandidates = (tracking, pkgSl) => {
    ...
    for (const tracking of universe) {
        ...
        for (const pkgDoc of pkgs) {
            ...
            let desiredInvId = winner?.id ?? null;
            let desiredInvNum = winner?.invoiceNumber ?? null;
            ...
            try {
                ...
                if (!currentInvId && desiredInvId)
                    linked++;
                else if (currentInvId && desiredInvId && currentInvId !== desiredInvId)
                    relinked++;
                else if (currentInvId && !desiredInvId)
                    cleared++;
            }
            catch (err) {
                v2_1.logger.warn("[invoice-trigger] package link patch failed", {
    ...
    if (linked || relinked || cleared) {
        v2_1.logger.info("[invoice-trigger] package link enforcement", {
 ...
 */
exports.onInvoiceWritten = (0, firestore_1.onDocumentWritten)({
    ...
    if (before && !after) {
        ...
        v2_1.logger.info("[invoice-trigger] Deletion detected", { invoiceId, invoiceNumber });
        await pushToSp2({
            id: invoiceId,
            invoiceNumber,
        ...
        await enforcePackageLinksForInvoice(invoiceId, { ...before, status: "deleted" }, before).catch((err) => {
            v2_1.logger.warn("[invoice-trigger] post-delete package re-link failed", { invoiceId, error: err.message });
        });
        return;
    }
    if (!after)
    ...
    if (patch && event.data?.after?.ref) {
        try {
            ...
            v2_1.logger.info("[invoice-trigger] Auto-healed legacy fields", {
                invoiceId, fields: Object.keys(patch),
            });
        }
        catch (err) {
            v2_1.logger.warn("[invoice-trigger] Auto-heal write failed", {
    ...
    if (afterSlCode && beforeSlCode !== afterSlCode) {
        v2_1.logger.info("[invoice-trigger] Invoice client reassignment detected", {
            invoiceId,
            fromSlCode: beforeSlCode,
            toSlCode: afterSlCode,
        });
        try {
            const customerDoc = await firebase_1.db.collection("customers").doc(afterSlCode).get();
            let custData = customerDoc.exists ? customerDoc.data() : null;
            let newCustomerId = customerDoc.exists ? customerDoc.id : null;
            if (!custData) {
                const fallbackSnap = await firebase_1.db.collection("customers").where("slCode", "==", afterSlCode).limit(1).get();
            ...
            if (custData && newCustomerId) {
                ...
                const trackings = Array.isArray(corrected.trackingNumbers) && corrected.trackingNumbers.length > 0
                    ...
                    : (Array.isArray(corrected.invoiceItems)
                        ? corrected.invoiceItems.map((it) => (it?.trackingNumber || it?.tracking || "")).filter((t) => !!t)
                        : []);
                if (trackings.length > 0) {
                    const batch = firebase_1.db.batch();
                    let pkgCount = 0;
                    const pkgSnaps = await Promise.all(trackings.map(t => firebase_1.db.collection("packages").where("trackingNumber", "==", t).get()));
                    ...
                    if (pkgCount > 0) {
                        await batch.commit();
                        v2_1.logger.info(`[invoice-trigger] Reassigned ${pkgCount} packages in SP1 to target client ${afterSlCode}`, { invoiceId });
                    }
                }
            }
        }
        catch (err) {
            v2_1.logger.warn("[invoice-trigger] package reassignment failed", {
                invoiceId, error: err.message,
            });
        }
    }
    ...
    if (SP2_EXCLUDED_STATUSES.has(afterStatus)) {
        ...
        if (wasActiveInSp2 || isAnnulledOrDeleted) {
            v2_1.logger.info("[invoice-trigger] Annulled/cancelled/deleted or downgrade — propagating delete to SP2", { invoiceId, status: afterStatus });
            await pushToSp2({
                id: invoiceId,
        ...
        await enforcePackageLinksForInvoice(invoiceId, corrected, before).catch((err) => {
            v2_1.logger.warn("[invoice-trigger] post-exclusion package re-link failed", { invoiceId, error: err.message });
        });
        return;
    }
    ...
    await enforcePackageLinksForInvoice(invoiceId, corrected, before).catch((err) => {
        v2_1.logger.warn("[invoice-trigger] package link enforcement failed", {
            invoiceId, error: err.message,
        });
    });
});
