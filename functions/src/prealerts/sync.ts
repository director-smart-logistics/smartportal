...
function isMLCargoCandidate(tracking: string): boolean {
 ...
 */
const TRACKING_SUFFIX_LENGTHS = [8, 10, 12] as const;
 ...
 */
function computeTrackingSuffixes(tracking: string | null | undefined): string[] {
  if (!tracking) return [];
  const digits = String(tracking).replace(/\D+/g, "");
  if (digits.length < TRACKING_SUFFIX_LENGTHS[0]) return [];
  const out: string[] = [];
  for (const len of TRACKING_SUFFIX_LENGTHS) {
    if (digits.length >= len) out.push(`L${len}:${digits.slice(-len)}`);
  }
  return out;
}

/** Legacy single-suffix accessor preserved for any historical reader. */
function computeTrackingSuffix12(tracking: string | null | undefined): string | null {
...
interface PortalSearchData {
  ...
  codigoCliente?: string; CodigoCliente?: string;
  nombreCliente?: string; NombreCliente?: string;
  shipper?: string; Shipper?: string;
  shipperDescripcion?: string; ShipperDescripcion?: string;
  peso?: number; Peso?: number;
  idManifest?: string; IdManifest?: string;
  descripcionArticulo?: string; DescripcionArticulo?: string;
  [key: string]: unknown;
}

interface MLCargoEnrichment {
  ...
  events: Array<{ fecha: string; ciudad: string; detalle: string }>;
  portalTracking: string | null;
  found: boolean;
}

interface SP2Shipment {
  id: string;
  tracking: string;
  slCode: string;
  userId: string;
  ...
  // sync metadata
  canonicalTracking?: string | null;
  lastMiddlewareCheck?: string | null;
}
...
function getSp2Firestore(): FirebaseFirestore.Firestore {
  ...
  if (existing) {
    sp2Db = getFirestore(existing);
  } else {
    const app = initializeApp({ projectId: SP2_PROJECT_ID }, appName);
...
function rawRequest(options: https.RequestOptions, body?: string, timeoutMs = 12_000): Promise<HttpResult> {
...
function extractCookies(headers: http.IncomingHttpHeaders): string {
  const sc = headers["set-cookie"] as string[] | string | undefined;
  if (!sc) return "";
...
async function ensureApiToken(): Promise<string> {
  if (apiTokenCache && Date.now() < apiTokenCache.expiresAt) return apiTokenCache.token;
  ...
  if (!parsed.token) throw new Error(`MLCargo auth failed: status=${res.status}`);
...
async function ensurePortalSession(): Promise<string> {
  if (portalSessionCache && Date.now() < portalSessionCache.expiresAt) return portalSessionCache.cookies;
  ...
  if (!csrfMatch) throw new Error("Portal: could not extract CSRF token");
...
function generateVariants(input: string): string[] {
  const cleaned = input.trim().toUpperCase().replace(/\s+/g, "");
  const seen = new Set<string>();
  const out: string[] = [];
  const add = (v: string) => { if (v && v.length >= 6 && !seen.has(v)) { seen.add(v); out.push(v); } };

  add(cleaned);

  // 20-22 digit USPS base → try 420+ZIP prefix
  if (/^9\d{19,21}$/.test(cleaned)) {
    for (const zip of MIAMI_ZIPS) add(`420${zip}${cleaned}`);
  }
  ...
  if (/^420\d{27}$/.test(cleaned)) {
    ...
    if (/^9/.test(base)) {
      add(base);
      for (const zip of MIAMI_ZIPS) add(`420${zip}${base}`);
    }
  }
  return out;
}
...
function detectMissingDest(destino: string, codigoCliente: string, manifestId: string): boolean {
  if (destino.toUpperCase() === "SD") return true;
  if (codigoCliente.toUpperCase() === "SD") return true;
  if (manifestId) {
    const n = manifestId.toUpperCase();
    if (n.includes("SINDESTINO")) return true;
    const s = n.match(/[A-Z]+$/)?.[0] ?? "";
    if (s === "DANS" || s === "SD" || s === "SIND") return true;
  }
  return false;
}
...
function extractCanonicalFromHtml(html: string, searchedTracking: string): string | undefined {
...
async function resolveViaPortal(
  originalTracking: string
): Promise<{ portalTracking: string | null; canonicalTracking: string | null; searchData: PortalSearchData | null }> {
  let cookies: string;
  try {
  ...
  if (!hit) return { portalTracking: null, canonicalTracking: null, searchData: null };

  // DetailByNumber with portal's tracking → extract canonical
  let canonicalTracking: string | null = null;
  try {
...
async function fetchMLCargoData(
  canonicalTracking: string,
  originalTracking: string
): Promise<Omit<MLCargoEnrichment, "canonicalTracking" | "portalTracking" | "found"> & { found: boolean }> {
  let token: string;
  try {
  ...
  if (infoRes?.status === 200) {
    try {
      ...
      if (p?.trackingNumber) packageInfo = p;
    } catch { /**/ }
  }
  if (eventsRes?.status === 200) {
    try {
      const p = JSON.parse(eventsRes.body);
      if (Array.isArray(p)) rawEvents = p;
    } catch { /**/ }
  }
...
async function enrichTracking(originalTracking: string): Promise<MLCargoEnrichment> {
  ...
  const portalWeight = searchData ? (searchData.peso ?? searchData.Peso ?? null) as number | null : null;
 ...
 */
function mergeStatusHistories(sp2History: any[], existingHistory: any[]): any[] {
  const seen = new Set<string>();
  const merged: any[] = [];
...
async function performPreAlertSync(forceFullSync = false): Promise<SyncStats> {
  ...
  await metaRef.set({ lastSyncAt: FieldValue.serverTimestamp() }, { merge: true });
  ...
  const sp2Docs: FirebaseFirestore.QueryDocumentSnapshot[] = [];
  for (const snap of [snap1, snap2]) {
    for (const doc of snap.docs) {
      if (!seenIds.has(doc.id)) { seenIds.add(doc.id); sp2Docs.push(doc); }
    }
  }
  console.log(`[PreAlertSync] SP2 returned ${sp2Docs.length} shipments (${snap1.size} prealert + ${snap2.size} smart_prealert)`);

  if (sp2Docs.length === 0) {
    await metaRef.set({ lastSyncAt: FieldValue.serverTimestamp(), totalProcessed: 0, stats }, { merge: true });
  ...
  const userIds = [...new Set(sp2Docs.map((d: FirebaseFirestore.QueryDocumentSnapshot) => str(d.data().userId)).filter(Boolean))];
  const userMap = new Map<string, SP2UserProfile>();
  ...
  const sp2DocIds = sp2Docs.map((d: FirebaseFirestore.QueryDocumentSnapshot) => d.id);
  const existingMap = new Map<string, SP1PreAlert>();
  for (let i = 0; i < sp2DocIds.length; i += 100) {
    ...
    for (const snap of existingSnaps) {
      if (snap.exists) existingMap.set(snap.id, snap.data() as SP1PreAlert);
    }
  }
  ...
  // Collect enriched payloads first, then batch-write atomically
  type WriteItem = { docId: string; payload: Record<string, any>; isNew: boolean };
  ...
  for (let i = 0; i < docs.length; i += SYNC_CONCURRENCY) {
    ...
    const results = await Promise.allSettled(chunk.map(async (doc) => {
      const sp2Doc = doc.data() as SP2Shipment;
      ...
      const user: SP2UserProfile = userMap.get(str(sp2Doc.userId)) ?? {
      ...
      let enrichment: MLCargoEnrichment | null = null;
      if (shouldEnrich) {
        try {
          enrichment = await enrichTracking(tracking);
          if (enrichment.found) stats.enriched++;
      ...
      const statusHistory = mergeStatusHistories(
        sp2Doc.statusHistory ?? [],
      ...
      return { docId: sp2DocId, payload: corePayload, isNew: !existing } as WriteItem;
    }));
    ...
    for (const result of results) {
      ...
      if (result.value) writeQueue.push(result.value);
    }
  }
  ...
  for (let i = 0; i < writeQueue.length; i += BATCH_LIMIT) {
    ...
    for (const item of slice) {
      ...
      if (item.isNew) {
        ...
        batch.set(ref, removeUndefined({ ...item.payload, createdAt: FieldValue.serverTimestamp() }));
        stats.created++;
      } else {
...
// SP2 user profile fetched during sync
...
// SP1PreAlert type for internal use
interface SP1PreAlert {
  id: string;
  sp2ShipmentId: string;
  sp2PreAlertId: string | null;
  sp2UpdatedAt: any;
  // Tracking
  tracking: string;
  canonicalTracking: string | null;
  carrier: string | null;
  // Customer (fully denormalized — no joins needed)
  slCode: string;
  userId: string;
  firstName: string;
  lastName: string;
  ...
  statusLabel: string;
  statusHistory: any[];
  middlewareEvents: any[];
  // Search tokens (prefix-indexed for starts-with queries)
  searchTokens: string[];
  // Sync metadata
 ...
 */
export const syncPreAlertsFromSP2 = onSchedule({
  ...
  console.log(
    `[PreAlertSync] Completed [${stats.mode}]: ` +
 ...
 */
export const triggerPreAlertSync = onCall({
  memory: "512MiB",
  timeoutSeconds: 1800,
}, async (request) => {
