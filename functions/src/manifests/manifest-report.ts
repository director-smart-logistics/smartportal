...
// ── Types (mirror of client-side LearningRecord) ──────────────────────────────
...
interface LearningRecord {
  ...
  multiMatchNames: string[];
  lowScoreNames: string[];
  rowsWithoutRoute: string[];
...
function buildBugsSection(bugs: BugReport[]): string {
  if (bugs.length === 0) {
    return `
...
function buildImprovementsSection(improvements: ImprovementSuggestion[]): string {
...
function buildUnmatchedSection(unmatchedNames: string[]): string {
  if (unmatchedNames.length === 0) return '';
  const shown = unmatchedNames.slice(0, 20);
  const extra = unmatchedNames.length - shown.length;
  const pills = shown.map(n =>
    `<span style="display:inline-block;background:#fee2e2;color:#991b1b;font-size:11px;font-weight:600;padding:3px 8px;border-radius:20px;margin:2px;">${n}</span>`
  ).join('');
...
function buildCorrectionsSection(corrections: LearningRecord['corrections']): string {
...
function buildEmailHtml(record: LearningRecord, recordId: string): string {
  const template = fs.readFileSync(
    path.join(__dirname, '..', '..', 'templates', 'manifest-report-email.html'),
    'utf8'
  );
...
function buildPlainText(record: LearningRecord, recordId: string): string {
...
export const slManifestReport = onCall(
  { region: 'us-central1', timeoutSeconds: 30 },
  async (request) => {
    const { record, recordId, to } = request.data as {
    ...
    if (!record || !recordId || !to) {
      throw new HttpsError('invalid-argument', 'Missing record, recordId or to');
    }

    logger.info('[slManifestReport] Sending manifest report email', {
    ...
    if (!result.success) {
      logger.error('[slManifestReport] Failed to send email', { error: result.error });
      throw new HttpsError('internal', `Email send failed: ${result.error}`);
    }

    logger.info('[slManifestReport] Report email sent', {
