...
import { RESEND_CONFIG, EMAIL_DEFAULTS } from './config';
...
function getResendClient(): Resend | null {
  if (!RESEND_CONFIG.apiKey) {
    logger.warn('[getResendClient] Resend API key not configured');
    return null;
  }
  
  if (!resendClient) {
    resendClient = new Resend(RESEND_CONFIG.apiKey);
    logger.info('[getResendClient] Resend client initialized');
  }
 ...
 */
function loadInvoiceEmailTemplate(): string {
  try {
    const templatePath = path.join(__dirname, '..', '..', 'templates', 'invoice-email.html');
    return fs.readFileSync(templatePath, 'utf8');
  } catch (error) {
    logger.error('[loadInvoiceEmailTemplate] Error loading template:', error);
 ...
 */
function replaceInvoiceTemplateVariables(template: string, data: InvoiceEmailData): string {
  let html = template;
 ...
 */
function generatePlainTextInvoice(data: InvoiceEmailData): string {
  const itemsList = data.items.map(item => 
    `- ${formatInvoiceItemCaption(item, data.source)}: ${data.currencySymbol}${item.amount.toFixed(2)}`
  ).join('\n');
...
INFORMACIÓN DE PAGO:
${EMAIL_DEFAULTS.companyName}
${EMAIL_DEFAULTS.companyId}
BAC Colones: ${EMAIL_DEFAULTS.bankAccountColones}
BAC Dólares: ${EMAIL_DEFAULTS.bankAccountDollars}
SINPE Móvil: ${EMAIL_DEFAULTS.sinpeMobile}

⚠️ IMPORTANTE: El pago debe realizarse en los próximos 2-3 días hábiles.

Gracias por confiar en SmartLogistics.
${EMAIL_DEFAULTS.website}
  `.trim();
}
...
export async function sendInvoiceEmail(data: InvoiceEmailData): Promise<{ success: boolean; error?: string; messageId?: string }> {
  ...
  if (!resend) {
    logger.error('[sendInvoiceEmail] Resend client not available');
    return { success: false, error: 'Resend client not configured. Set RESEND_API_KEY environment variable.' };
  }
  
  if (!data.customerEmail) {
    logger.warn('[sendInvoiceEmail] No customer email provided');
  ...
  try {
    ...
    const { data: responseData, error } = await resend.emails.send({
      from: `${RESEND_CONFIG.invoiceSenderName} <${RESEND_CONFIG.invoiceSenderEmail}>`,
      to: [data.customerEmail],
      replyTo: RESEND_CONFIG.invoiceSenderEmail,
    ...
    if (error) {
      logger.error('[sendInvoiceEmail] Resend API error:', {
        to: data.customerEmail,
    ...
    logger.info('[sendInvoiceEmail] Invoice email sent successfully:', {
    ...
    logger.error('[sendInvoiceEmail] Error sending invoice email:', {
      to: data.customerEmail,
      invoiceNumber: data.invoiceNumber,
...
export async function getEmailStatus(
...
): Promise<{ success: boolean; status?: EmailDeliveryStatus; error?: string }> {
  const resend = getResendClient();
  ...
  try {
    ...
    if (error) {
      logger.error('[getEmailStatus] Resend API error:', {
    ...
    const status: EmailDeliveryStatus = {
      id: (data as any)?.id || resendMessageId,
      from: (data as any)?.from || '',
      to: (data as any)?.to || [],
      subject: (data as any)?.subject || '',
      created_at: (data as any)?.created_at || '',
      last_event: (data as any)?.last_event || 'unknown',
    };

    logger.info('[getEmailStatus] Status retrieved:', {
      messageId: resendMessageId,
    ...
    logger.error('[getEmailStatus] Error checking status:', {
      messageId: resendMessageId,
      error: error?.message || 'Unknown error',
    });
    return { success: false, error: error?.message || 'Unknown error' };
...
export async function getEmailStatusBatch(
  messageIds: string[]
): Promise<Array<{ messageId: string; success: boolean; status?: EmailDeliveryStatus; error?: string }>> {
  const results: Array<{ messageId: string; success: boolean; status?: EmailDeliveryStatus; error?: string }> = [];
 ...
 */
export async function sendWelcomeEmail(data: WelcomeEmailData): Promise<{ success: boolean; error?: string; messageId?: string }> {
  const resend = getResendClient();

  if (!resend) {
    logger.warn('[sendWelcomeEmail] Resend client not available — skipping welcome email');
    return { success: false, error: 'Resend client not configured' };
  }
  ...
  try {
    ...
    const { data: responseData, error } = await resend.emails.send({
      from: `DA SmartLogistics Costa Rica - Depto. De Tecnología <${RESEND_CONFIG.senderEmail}>`,
      to: [data.email],
    ...
    if (error) {
      logger.error('[sendWelcomeEmail] Resend error', { to: data.email, error: error.message });
      return { success: false, error: error.message };
    }

    logger.info('[sendWelcomeEmail] Welcome email sent', { to: data.email, messageId: responseData?.id || messageId });
    return { success: true, messageId: responseData?.id || messageId };

  } catch (err: any) {
    logger.error('[sendWelcomeEmail] Error sending welcome email', { to: data.email, error: err?.message });
 ...
 */
export async function sendEmail(data: GenericEmailData): Promise<{ success: boolean; error?: string; messageId?: string }> {
  const resend = getResendClient();
  ...
  try {
    ...
    const { data: responseData, error } = await resend.emails.send({
      ...
      to: [data.to],
      replyTo: data.replyTo || RESEND_CONFIG.replyTo,
