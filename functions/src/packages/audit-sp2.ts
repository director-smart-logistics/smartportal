...
export const slAuditSp2Package = onCall(
  { cors: true },
  async (request: CallableRequest<AuditRequest>): Promise<any> => {
    if (!request.auth) {
    ...
    try {
      ...
      for (const pkgItem of localPackages) {
        ...
        try {
          ...
          logger.warn(`[slAuditSp2Package] Optional direct document fetch failed for tracking ${pkgTracking}:`, { error: docErr.message });
        }
        ...
        auditRes.packages.push({
          ...
          statusSp2,
          mismatch,
          sp2Docs: shipmentDocs.map((doc: any) => ({
            id: doc.id,
      ...
      for (const localInv of invoicesList) {
        ...
        try {
          ...
          logger.warn(`[slAuditSp2Package] Failed checking SP2 invoice ${invId}:`, { error: err.message });
        }
      ...
      logger.error(`[slAuditSp2Package] General audit execution failed for tracking ${tracking}:`, error);
      return {
        success: false,
...
export const slDeleteSp2Shipment = onCall(
  { cors: true },
  async (request: CallableRequest<{ shipmentId: string }>): Promise<any> => {
    if (!request.auth) {
      throw new HttpsError("unauthenticated", "User must be authenticated");
    }
    ...
    try {
      ...
      logger.error(`[slDeleteSp2Shipment] Failed to delete shipment ${shipmentId}:`, error);
      return {
        success: false,
