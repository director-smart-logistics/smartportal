...
export const slLookupCustomerByEmail = onRequest(
  ...
  async (req, res) => {
    ...
    const incoming = req.headers["x-sync-secret"] as string | undefined;
    const expected = process.env[SECRET_ENV];
    if (!expected || incoming !== expected) {
      logger.warn("[slLookupCustomerByEmail] unauthorized", { ip: req.ip });
      res.status(401).json({ success: false, error: "Unauthorized" });
      return;
    }

    const raw = (req.method === "GET" ? req.query.email : req.body?.email) as string | undefined;
    ...
    try {
      ...
      const [s1, s2] = await Promise.all([
        db.collection("customers").where("email", "==", email).limit(2).get(),
        email !== raw
          ? db.collection("customers").where("email", "==", raw).limit(2).get()
      ...
      logger.error("[slLookupCustomerByEmail] lookup failed", {
