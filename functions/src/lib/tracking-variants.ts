...
export function buildTrackingVariants(raw: string): string[] {
  ...
  if (digitsOnly && digitsOnly !== upper) set.add(digitsOnly);

  const upsMatch = upper.match(/1Z[A-Z0-9]{16}/);
  if (upsMatch) set.add(upsMatch[0]);
  ...
  if (uspsMatch) set.add(uspsMatch[0]);

  const uspsExpressMatch = upper.match(/[A-Z]{2}[0-9]{9}US/);
  if (uspsExpressMatch) set.add(uspsExpressMatch[0]);
  ...
  if (digitsOnly.startsWith("420")) {
    for (const offset of [8, 12]) {
      if (digitsOnly.length > offset + 12) {
        ...
        set.add(stripped);
        if (stripped.length >= 20) set.add(stripped.slice(-20));
        if (stripped.length >= 22) set.add(stripped.slice(-22));
        const innerMatch = stripped.match(/9[0-9]{19,21}/);
        if (innerMatch) set.add(innerMatch[0]);
      }
    }
  }
  ...
  if (gs1Numeric) {
    for (const seg of gs1Numeric.slice(0, 5)) set.add(seg);
  }

  for (const len of [22, 20, 18, 15, 12]) {
    if (digitsOnly.length > len) set.add(digitsOnly.slice(-len));
