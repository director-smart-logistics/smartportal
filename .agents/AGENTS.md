...
## 17. Cross-Manifest Package Invariants & Foreign Manifest Collision Guard (Strict Non-Bypassable)
- **Problem & Post-Mortem (Cross-Manifest Package Theft)**: When packages are moved or consolidated between manifests (e.g., from `SL-MEGA-MAN-17-09-2026` or `17-09-2026DAN` to `18-09-2026DAN`), their `packages/{tracking}` Firestore document is updated to the destination manifest. However, the source manifest document `manifests/{id}` may still retain the tracking in its embedded `packages[]` JSON array. If the source manifest or Mega-Man was reloaded in Nova, improper query lookups (`where('trackingNumber', 'in', ...)` which fails on docId-keyed records) caused `loadMegaManFromFirestore` to treat them as missing and re-inject them via `embeddedSupplement`. Autosave (`useNovaAutoSave`) or manual save would then overwrite `packages/{tracking}.manifestNumber` back to the source manifest, corrupting invoices and stealing packages from the destination manifest.
- **Three-Tier Invariant Enforcement**:
  1. **Direct Document-ID Lookups in `fusion.ts`**: Never query `packages` by `where('trackingNumber', 'in', ...)` for candidate tracking verification. Always query deterministically via `getDoc(doc(db, 'packages', trackingId))`.
  2. **Strict Manifest Whitelist (`targetMnSet`) in `fusion.ts`**: Candidate supplement packages loaded from `manifests.packages[]` MUST be verified against `targetMnSet = { megaManId, ...searchTerms }`. If a package exists in `packages/{tracking}` and its `manifestNumber` belongs to an active foreign manifest (e.g. `18-09-2026DAN`), it **MUST BE STRICTLY EXCLUDED** from the in-memory hydration.
  3. **Write-Time Invariant Guard in `ingestion.ts`**: In `upsertManifestPackageOverrides` and `ingestManifestToPackages`, the system constructs `allowedSourceSet = { manifestNumber, ...allowedSourceManifests, ...row.manifiesto }`. If a package in Firestore has a `currentManifest` that is NOT in `allowedSourceSet` and there is no explicit `rowManifestOverrides[trackingId]`, the write operation **MUST STRICTLY SKIP WRITING / OVERWRITING** `manifestNumber` and package overrides.
  4. **Bidirectional Parent Container Pruning**: When transferring packages between manifests (`movePackagesBetweenManifestDocs`), the system must inspect source and destination manifests for parent `MEGA-MAN` or `ENC-MEGA-MAN` containers and atomically prune the moved trackings from their embedded `packages[]` arrays to keep snapshots in sync.
- **Testing Mandate**: Any modification to manifest loading, saving, fusions, or carry-on must pass all unit tests in `manifest-processor.round-trip.spec.ts` and maintain 100% clean passes across the full test suite.
