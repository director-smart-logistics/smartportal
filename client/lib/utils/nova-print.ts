...
export function buildRouteManifestHTML(
  ...
  const groupsHtml = Array.from(groupMap.values())
    ...
    g.rows.forEach(r => {
      ...
      if (r.invoiceId || r.invoiceNumber) {
        ...
        if (!activeInvoiceMap.has(invKey)) {
          const invUsd = r.invoiceAmountUSD != null ? (Number(r.invoiceAmountUSD) || 0) : rowPrice;
          const invCrc = r.invoiceAmountCRC != null ? (Number(r.invoiceAmountCRC) || 0) : undefined;
        ...
        uninvoicedPriceSum += rowPrice;
      }
    });

    const activeInvoices = Array.from(activeInvoiceMap.values());
    const activeInvoicesUSD = activeInvoices.reduce((s, inv) => s + (Number(inv.usd) || 0), 0);
    const total = activeInvoices.length > 0
      ? (activeInvoicesUSD + uninvoicedPriceSum)
      : g.rows.reduce((s, r) => s + (Number(r.price) || 0), 0);
    
    let totalCRC = '';
    if (numericTc > 0) {
      const activeInvoicesCRC = activeInvoices.reduce((s, inv) => s + (inv.crc != null ? (Number(inv.crc) || 0) : ((Number(inv.usd) || 0) * numericTc)), 0);
      const groupCRC = activeInvoices.length > 0 ? (activeInvoicesCRC + Math.round(uninvoicedPriceSum * numericTc)) : Math.round((Number(total) || 0) * numericTc);
    ...
    const childRows = g.rows.map((r, i) => {
      ...
      if (hasReturnedInGroup) {
        ...
        const priceCRC = numericTc > 0 ? Math.round(priceUSD * numericTc) : 0;
        amtHtml = `<div class="child-amt-split">
             <span class="child-usd">$${priceUSD.toFixed(2)}</span>
             ${numericTc > 0 ? `<span class="child-crc">&#8353;${priceCRC.toLocaleString('es-CR')}</span>` : ''}
           </div>`;
      }
            ...
            <td class="total-amt">
              <div class="total-split">
                <span class="total-usd-block">
                  <span class="total-line">$${(Number(total) || 0).toFixed(2)}</span>
                </span>
  ...
  const grandTotal    = (filteredRows || []).reduce((s, r) => s + (Number(r.price) || 0), 0);
  const grandTotalCRC = numericTc > 0 ? ` · ₡${Math.round(grandTotal * numericTc).toLocaleString('es-CR')}` : '';
...
<body>
  <header>
    ...
    <div style="text-align:right;">
      ${numericTc > 0 ? `<p class="tc-global"><strong>Tipo de Cambio: &#8353;${numericTc.toLocaleString('es-CR')}</strong></p>` : ''}
  ...
  <footer>
    ...
    <span>Total: $${(Number(grandTotal) || 0).toFixed(2)}${grandTotalCRC}</span>
  </footer>
</body>
</html>`;
}
...
export function buildEncomiendaServiceManifestHTML(
  ...
  const servicesHtml = sortedServices.map(([serviceName, cMap]) => {
    const serviceTotal    = Array.from(cMap.values()).reduce((s, c) => s + c.rows.reduce((rs, r) => rs + (Number(r.price) || 0), 0), 0);
    const serviceTotalCRC = numericTc > 0 ? ` · &#8353;${Math.round(serviceTotal * numericTc).toLocaleString('es-CR')}` : '';

    const customersHtml = Array.from(cMap.values())
      .sort((a, b) => a.customerName.localeCompare(b.customerName, 'es', { sensitivity: 'base' }))
      .map(c => {
        const custTotal    = c.rows.reduce((s, r) => s + (Number(r.price) || 0), 0);
        const custTotalCRC = numericTc > 0 ? `<span class="crc">&#8353;${Math.round(custTotal * numericTc).toLocaleString('es-CR')}</span>` : '';
        const rowspan      = c.rows.length + 1;
        const pkgRows = c.rows.map((r, i) => {
          const pesoNum = Number(r.peso) || 0;
          return `
          <tr class="child ${i % 2 === 0 ? 'even' : 'odd'}">
            <td class="center small">${i + 1}</td>
            ...
            <td class="center">${pesoNum > 0 ? `${pesoNum.toFixed(2)} kg` : '<span class="na">—</span>'}</td>
            <td class="sig"></td>
          </tr>`;
        }).join('');
        return `
        <tbody>
          <tr class="customer-header">
            ...
            <td class="total-amt">
              <span class="total-line">$${(Number(custTotal) || 0).toFixed(2)}</span>
              ${custTotalCRC}
            </td>
            <td class="pago-cell" rowspan="${rowspan}"><span class="pago-opt"><span class="chk">&#9744;</span>&nbsp;Ef</span><span class="pago-opt"><span class="chk">&#9744;</span>&nbsp;Tr</span><span class="pago-opt"><span class="chk">&#9744;</span>&nbsp;Sinpe</span></td>
          </tr>
    ...
    <tbody>
      <tr class="service-header">
        <td colspan="6" class="service-name">${serviceName}<span class="service-total">&nbsp;— $${(Number(serviceTotal) || 0).toFixed(2)}${serviceTotalCRC}</span></td>
      </tr>
    </tbody>
    ${customersHtml}`;
  }).join('');

  const grandTotal    = (rows || []).reduce((s, r) => s + (Number(r.price) || 0), 0);
  const grandTotalCRC = numericTc > 0 ? ` · &#8353;${Math.round(grandTotal * numericTc).toLocaleString('es-CR')}` : '';
...
<body>
  <header>
    ...
    <div style="text-align:right;">
      ${numericTc > 0 ? `<p class="tc-global"><strong>TC: &#8353;${numericTc.toLocaleString('es-CR')}</strong></p>` : ''}
    </div>
  </header>
  <table>
    <colgroup>
